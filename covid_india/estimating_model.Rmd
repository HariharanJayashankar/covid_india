---
title: " Estimating $R_0$ by State for India"
...
We want to estimate $R_0$. I follow a simple linear hierarchical model here. For a given state,
we guess that the total number of cases $x$ follows this process:

$$x_{t+5} = R_0x_t + \epsilon_t$$
...
have large confidence intervals. Also we want to incorporate prior knowledge of the parameter $R_0$ into our data. Zhang et al ([link](https://www.ncbi.nlm.nih.gov/pubmed/32097725)) estimate an $R_0$ of 2.28 for covid-19 from the Diamond Princess.

Because of this we use a basic hierarchical model from the bayesian literature to get estimates of $R_0$ by states.
...
$$\mu_{t+5,j} = R_{0, j}\mu_{t,j} + \beta_0$$
...
As of now, parameters for the priors have been chosen a bit at random except for $r_j$ whos mean is chosen according to [Zhang et al](https://www.ncbi.nlm.nih.gov/pubmed/32097725).
